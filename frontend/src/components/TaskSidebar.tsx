...
function TaskSidebar({ task, onClose, initialPlayerId }: { task: Task; onClose: () => void; initialPlayerId: string | null }) {
  ...
  const fetchCoachResources = async () => {
    ...
    try {
      const res = await fetch(`/api/posts/coachResources/${teamInfo.team_id}/${task.task_id}`, {
        credentials: 'include'
      });
      if (!res.ok) {
  ...
  const fetchMyMedia = async () => {
    ...
    try {
      const res = await fetch(`/api/posts/myMedia/${teamInfo.team_id}/${task.task_id}`, {
        credentials: 'include'
      });
      if (!res.ok) {
  ...
  const fetchSubmissionStatus = async () => {
    ...
    try {
      const res = await fetch(`/api/tasks/${teamInfo.team_id}/status/${task.task_id}`, {
        method: 'GET',
        credentials: 'include'
      });
      if (!res.ok) {
  ...
  const playerSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      const response = await fetch(`/api/tasks/${teamInfo.team_id}/submit/${task.task_id}`, {
        method: "POST",
        credentials: 'include'
      });
      if (!response.ok) {
  ...
  const unsubmitTask = async () => {
    ...
    try {
      const response = await fetch(`/api/tasks/${teamInfo.team_id}/unsubmit/${task.task_id}`, {
        method: "DELETE",
        credentials: 'include'
      });
      if (!response.ok) {
  ...
  const handleSaveDescription = async () => {
    ...
    try {
      const response = await fetch(`/api/tasks/${teamInfo.team_id}/description/${task.task_id}`, {
        method: "PATCH",
        headers: {
